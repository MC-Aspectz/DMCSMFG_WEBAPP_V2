...
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        ...
        MARKETCD = item.eq(1).text();

        $('#MARKETCD').html(item.eq(1).text());
        $('#MARKETNAME').html(item.eq(2).text());
    }
...
function HandleResult(result) {
    try {
        window.opener.HandlePopupResult('MARKETCD', result);
    } catch (err) {
        // console.log(err);
    }
    $('#loading').hide();
