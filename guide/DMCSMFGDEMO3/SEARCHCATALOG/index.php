  ...
  <form class="w-full h-screen p-2" method="POST" id="itemcategory" name="itemcategory" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-6/12">
              <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['categorycode']; ?></label>
                ...
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-3/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['category']; ?></span>
                        </th>
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['categoryname']; ?></span>
                        </th>
                        <th class="w-5/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['description']; ?></span>
                        </th>
                    </tr>
                </thead>
                    ...
                    if (!empty($tdata)) { $minrow = count($tdata) + 1;
                        foreach ($tdata as $item) { ?>
                            <tr class="flex w-full p-0 divide-x">
                                <td class="h-6 w-3/12 text-sm pl-1"><?=$item['CATALOGCD'] ?></td>
                                <td class="h-6 w-4/12 text-sm pl-1"><?=$item['CATALOGNAME'] ?></td>
                                <td class="h-6 w-5/12 text-sm pl-1"><?=$item['CATALOGDESC'] ?></td>
                            </tr><?php 
                        }
                    }
      ...
      <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['category'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="categorycd"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['categoryname'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="categoryname"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['description'] ?></td>
