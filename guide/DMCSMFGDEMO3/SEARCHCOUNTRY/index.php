...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['countryindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="guideindex" name="guideindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        ...
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['countryname']; ?></label>
                ...
                <thead class="sticky top-0 z-20 bg-gray-50">
                    <tr class="border border-gray-600">
                        <th class="px-6 text-center border border-slate-700 text-left">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['countrycd']; ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['countryname']; ?></span>
                        </th>
                    </tr>
                </thead>
                    ...
                    if (!empty($tdata)) { $minrow = count($tdata);
                        foreach($tdata as $key => $item) { ?>
                            <tr class="row-id">
                                ...
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=isset($item['COUNTRYCD']) ? $item['COUNTRYCD']: '' ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=isset($item['COUNTRYNAME']) ? $item['COUNTRYNAME']: '' ?></td>
                            </tr><?php 
                        }
                    } 
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                ...
                <tbody>
                    <tr>
                        <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=$lang['countrycd'] ?></td>
                        <td class="text-left pl-1 border border-slate-700 text-sm" id="countrycd" style="background-color:#ffe6cc;"></td>
                    </tr>
                    <tr>
                        <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['countryname'] ?></td>
                        <td class="text-left pl-1 border border-slate-700 text-sm" id="countryname"></td>
                    </tr>
                </tbody>
            </table>
            <br>
