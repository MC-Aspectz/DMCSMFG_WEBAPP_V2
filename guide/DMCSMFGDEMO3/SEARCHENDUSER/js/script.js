//  form
const form = document.getElementById('enduserindex');

var page = $('#page').val();
var ENDUSERCD; var isItem = false;
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        ...
        isItem = true;
        ENDUSERCD = item.eq(1).text();
        
        $('#ENDUSER').html(item.eq(1).text());
        $('#ENDUSERNAME').html(item.eq(2).text());
    }
    
    $('#select_item').on('click', function() {
        $('#loading').show();
        return HandleResult(ENDUSERCD);
    });
});
...
function HandleResult(result) {
    try {
        window.opener.HandlePopupResult('ENDUSERCD', result);
    } catch (err) {
        // console.log(err);
    }
    $('#loading').hide();
