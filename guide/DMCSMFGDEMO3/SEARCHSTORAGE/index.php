...
<head>
    ...
    <title><?php echo $_SESSION['APPNAME'].' - '.$lang['storageindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="itemlocation" name="itemlocation" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['locationcd']; ?></label>
        ...
        <div class="table">
                ...
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-4/12 text-left pl-1">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['locationcd']; ?></span>
                        </th>
                    ...
                    if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                        foreach ($tdata as $item) { ?>
                            <tr class="flex w-full p-0 divide-x">
                                ...
                                <td class="h-6 w-4/12 text-sm pl-1"><?=$item['STORAGECD'] ?></td>
                                <td class="h-6 w-8/12 text-sm pl-1"><?=$item['STORAGECD'] ?></td>
                            </tr><?php 
                        }
                    }
            ...
            <div class="flex p-2">
                <label class="text-color h-6 text-[12px]"><?php echo $lang['rowcount']; ?>  <span id="rowcount"><?=$minrow?></span></label>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td lass="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=$lang["locationcd"] ?></td>
                            <td lass="text-left pl-1 border border-slate-700 text-sm" id="locationcd" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                            <td lass="text-left pl-1 border border-slate-700 text-sm"><?=$lang["locationname"] ?></td>
                            <td lass="text-left pl-1 border border-slate-700 text-sm" id="locationname"></td>
                        </tr>
                    </tbody>
                </table>
                <br>
