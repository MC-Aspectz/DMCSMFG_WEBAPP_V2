...
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        isItem = true;
        STORAGECD = item.eq(1).text();
    }

    $('#locationcd').html(item.eq(1).text());
    $('#locationname').html(item.eq(2).text());

    $('#select_item').on('click', function() {
        $('#loading').show();
        return HandleResult(STORAGECD);
    });
});
...
function HandleResult(result) {
    try {
        window.opener.HandlePopupResult('STORAGECD', result);
    } catch (err) {
        // console.log(err);
    }
    $('#loading').hide();
