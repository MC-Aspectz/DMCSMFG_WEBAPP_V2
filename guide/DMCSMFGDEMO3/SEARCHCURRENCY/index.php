...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['currencyindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="guideindex" name="guideindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['currency']; ?></label>
                ...
                <thead class="sticky top-0 z-20 bg-gray-50">
                    <tr class="border border-gray-600">
                        <th class="px-6 text-center border border-slate-700 text-left">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['currencycode']; ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['decimalunit']; ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['decimaltotal']; ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['currencydisplayed']; ?></span>
                        </th>
                    </tr>
                </thead>
                <tbody id="dvwdetail" class="divide-y divide-gray-200"><?php
                    if (!empty($tdata)) { $minrow = count($tdata);
                        foreach($tdata as $key => $item) {
                            ...
                            $currencyunitdisp = '';
                            $currencyamtdisp = '';
                            if ($item['CURRENCYUNITTYP'] != '') {
                                $currencyunitdisp = 1 / pow(10, $item['CURRENCYUNITTYP']);
                            }  // if ($item['CURRENCYUNITTYP'] != '') {
                            if ($item['CURRENCYAMTTYP'] != '') {
                                $currencyamtdisp = 1 / pow(10, $item['CURRENCYAMTTYP']);
                            }  // if ($item['CURRENCYAMTTYP'] != '') {
                            // -------------------------------------------------- ?>
                            <tr class="row-id">
                                <td class="hidden"><?=$key; ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['CURRENCYCD'] ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$currencyunitdisp ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$currencyamtdisp ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['CURRENCYDISP'] ?></td>
                            </tr><?php 
                        }
                    } 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=$lang['currencycode'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="currencycode" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['decimalunit'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="decimalunit"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=$lang['decimaltotal'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="decimaltotal" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['currencydisplayed'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="currencydisplayed"></td>
                        </tr>
                    </tbody>
                </table>
                <br>
