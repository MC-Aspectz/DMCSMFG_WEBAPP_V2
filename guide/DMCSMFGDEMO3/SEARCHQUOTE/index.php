...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.lang('quotationindex'); ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="quotationindex" name="quotationindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=lang('CUSTOMERCODE'); ?></label>
                ...
                <input type="text" class="text-control text-sm shadow-md border rounded-xl h-7 w-5/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 read"
                        name="P1NAME" id="P1NAME" value="<?=isset($P1NAME) ? $P1NAME: '' ?>" readonly/>
            </div>
        ...
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=lang('ESTIMATE_NO'); ?></label>
                <input type="text" class="text-control text-sm shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                        id="P2" name="P2" value="<?=isset($P2) ? $P2: '' ?>"/>
            </div>
            <div class="flex w-5/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1 hidden"><?=lang('ESTTMNAME'); ?></label>
                <input type="text" class="text-control shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 hidden"
                        id="P5" name="P5" value="<?=isset($P5) ? $P5: '' ?>"/>
            </div>
        </div>

        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=lang('HEADER_DATE'); ?></label>
                <input type="date" class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
                        id="P3" name="P3" value="<?=!empty($P3) ? date('Y-m-d', strtotime($P3)) : ''?>"/>&ensp;→&ensp;
                <input type="date" class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
                        id="P4" name="P4" value="<?=!empty($P4) ? date('Y-m-d', strtotime($P4)) : ''?>"/>
            </div>
            <div class="flex w-5/12 justify-end">
                <button type="button" class="btn text-color inline-flex items-center border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2"
                        id="SEARCH" name="SEARCH"><?=lang('SEARCH'); ?>
                ...
                <thead class="sticky top-0 z-20 bg-gray-50">
                    <tr class="border border-gray-600">
                        <th class="px-6 text-center border border-slate-700 text-left">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('ESTIMATE_NO'); ?></span>
                        </th>
                        ...
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('ESTTMNAME'); ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('DELI_PLACE'); ?></span>
                        </th>
                        <th class="px-6 text-center border border-slate-700 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('DELE_PLACE_NAME'); ?></span>
                        </th>
                    </tr>
                </thead>
                    ...
                    if (!empty($tdata)) { $minrow = count($tdata);
                        foreach($tdata as $key => $item) { ?>
                            <tr class="row-id">
                                ...
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['ESTNO'] ?></td>
                                ...
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['ESTTITLE'] ?></td>
                                <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['DELIVERYCD'] ?></td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=lang('ESTIMATE_NO'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ESTIMATE_NO"></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=lang('ESTTMNAME'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ESTTMNAME"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=lang('DELI_PLACE'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="DELI_PLACE"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=lang('DELE_PLACE_NAME'); ?></td>
