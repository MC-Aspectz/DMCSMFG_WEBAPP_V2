...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.$lang['jobcodeindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="jobcodeindex" name="jobcodeindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-6/12 px-2">
                <label class="text-color block text-sm w-2/12 pr-2 pt-1"><?=$lang['jobcode']; ?></label>
                ...
                <!-- <label class="text-color block text-sm w-2/12 pt-1 text-center"><?=$lang['jobname']; ?></label> -->
                &ensp;→&ensp;
                ...
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['jobcode']; ?></span>
                        </th>
                        <th class="w-8/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['jobname']; ?></span>
                        </th>
                    </tr>
                </thead>
                ...
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-4/12 pl-1 text-sm"><?=isset($item['JOBCD']) ? $item['JOBCD']: '' ?></td>
                            <td class="h-6 w-8/12 pl-1 text-sm"><?=isset($item['JOBNAME']) ? $item['JOBNAME']: '' ?></td>
                        </tr><?php 
                    }
                }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['jobcode'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="jobcode"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang["jobname"] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="jobname"></td>
                        </tr>
