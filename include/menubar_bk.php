...
function navBar() { 
    ...
    <!-- <script src="<?=$_SESSION['APPURL'] . '/js/menu.js'; ?>" integrity="sha384-CM+ab6Ns/541eGh4lxRDBdRVi8G/CCOrIn25/FxOnPzHaH1ARQGvm6DONwYSarGI" crossorigin="anonymous"></script> -->
    <script src="<?=$_SESSION['APPURL'] . '/js/general.js'; ?>" integrity="sha384-jw+panvIBzQkPmLGowaauiVkxSebSfh3EXxigHKWqNlHB1DrpldwG6FUIKU93yiL" crossorigin="anonymous"></script>
    ...
    <script type="text/javascript"> 
        ...
        // console.log(window.location.href.includes('/app/DMCSMFGDEMO3/'));
        // console.log(window.location.href.indexOf('/app/DMCSMFGDEMO3/'));
        // if (window.location.href.indexOf('/app/DMCSMFGDEMO3/') > -1) {
        //     console.log('your url contains the name DMCSMFGDEMO3');
        // }
...
$(document).mouseenter(function () {
    ...
    $(window).on('mouseout', (async function () {
        window.addEventListener('blur', async function() {

        });
    }));
            
if (!window.navigator.userActivation.isActive) {  // close
    console.log('close');
} else { // refresh 
    console.log('refresh');
}
...
window.addEventListener('beforeunload', function (e) {
    ...
    // debugger;
            ...
            // $('.text-control').on('keyup change', function(e) {
            //     if ((e.type === 'change') || (e.key === 'Enter' || e.keyCode === 13)) {
            //         isRefresh = true;
            //     }
            // });

const pageReload = ((PerformanceNavigationTiming && PerformanceNavigationTiming.TYPE === 1) || window.performance.getEntriesByType('navigation').map((nav) => nav.type).includes('reload'));
    ...
    // };
        ...
        $(document).ready(function() {
            ...
            if (brightness.isDark()) {
                ...
                if(appmod != '') document.getElementById('mod-'+appmod+'').classList.add('bg-gray-700');
            } else {
                for(var seq = 1; seq <= 13; seq++) {
                    if( seq < 10 ) { seq = '0'+seq } else { seq = seq; };
                ...
                if(appmod != '') document.getElementById('mod-'+appmod+'').classList.add('bg-sky-100');
            }
            // -------------------------------------------------------------------- //
            // ------------------------------ Main body --------------------------- //
            const main = document.querySelector('main');
            if(mainbgtyper == 'Picture') {
                const bgImgPath = appurl + '/img/background/' + companycode + '/' + usercode + '/' + mainbgvalue;
...
function sideBar() {
    ...
    //--------------------------------------------------------------------------------
    // Get App Module
    $appmod = $_SESSION['APPMOD'];
    $appcode = isset($_SESSION['APPCODE']) ? $_SESSION['APPCODE']: '';
    $findpack = array_filter($_SESSION['MENU'], function($v, $k) use ($appmod) {
      return $v['MODSEQ'] == $appmod && $v['FORMAPP'] == 10 && $v['FORMSEQ'] == 0;
    }, ARRAY_FILTER_USE_BOTH);

    if(!empty($findpack)) {
        if(empty($_SESSION['PACKCODE']) || $_SESSION['APPCODE'] == '') {
    ...
    <aside class="side-menu flex-shrink-0 w-64 flex flex-col border-r transition-all duration-300 overflow-y-auto">
        <nav class="flex flex-col flex-wrap p-2 w-full" data-hs-accordion-always-open>
            ...
            foreach ($findpack as $value) { if($value['NODEDATA'] == $_SESSION['PACKCODE']) { ?>
                        ...
                        foreach ($appdata as $item) {
                            ...
                            <li class="py-0.5">
                                <a class="subtopic flex items-start gap-x-3.5 py-1 px-2.5 text-[12px] rounded-lg cursor-pointer" id="<?=$item['NODEDATA']?>" 
                                    onclick="javascript:onloadpage(); changeApp('<?=$item['NODEKEY']?>', '<?=$_SESSION['COMCD']?>', '<?=$appcode?>');"><?=$item['NODETITLE']?></a>
                            </li><?php
                            }
                        } ?>
                        </ul>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            // ------------------------------ Sidebar ----------------------------- //
            const appcode = '<?php echo isset($_SESSION['APPCODE']) ? $_SESSION['APPCODE']: '' ?>';
            // console.log(appcode);
            ...
            Array.from(document.getElementsByClassName('subtopic')).forEach(app => {
                if(tinycolor(sidebgcolor).isDark()) {
                    ...
                    if((appcode != '' && appcode != 'ACC_RESALEENTRY_THA') || (appcode != '' && appcode != 'ACC_INCVO')) {
