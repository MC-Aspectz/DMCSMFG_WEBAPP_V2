// button search
const searchdirect = $("#searchdirect");

searchdirect.attr(
  "href",
  $("#sessionUrl").val() + "/guide/SEARCHDIRECT/index.php"
);

//input serach
const DIRECTCD = $("#DIRECTCD");

const input_serach = [DIRECTCD];
...
// form
const form = document.getElementById("direct_master");
...
// searchdirect.click(function() {
//     keeyData();
// });

DIRECTCD.change(function () {
  window.location.href = "index.php?DIRECTCDS=" + DIRECTCD.val();
});

DIRECTCD.keyup(function (e) {
  if (e.key === "Enter" || e.keyCode === 13) {
    window.location.href = "index.php?DIRECTCDS=" + DIRECTCD.val();
  }
});
...
$("table#search_table tr").click(function () {
  ...
  if (item.eq(0).text() != "undefined" && item.eq(0).text() != "") {
    ...
    $("#DIRECTCD").val(item.eq(0).text());
    $("#DIRECTNAME").val(item.eq(1).text());
...
async function commit(method) {
  ...
  await axios
    .post("../DIRECTMASTER/function/index_x.php", data)
    .then((response) => {
...
async function clearForm(form) {
  ...
  emptyRow();
  // refresh
  window.location.href = "../DIRECTMASTER/";
  return false;
}
