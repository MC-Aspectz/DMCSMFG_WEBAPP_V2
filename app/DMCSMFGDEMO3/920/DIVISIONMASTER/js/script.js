...
// form
const form = document.getElementById("division_master");
...
DIVISIONCD.change(function () {
  window.location.href = "index.php?DIVISIONCDS=" + DIVISIONCD.val();
});
...
$("table#search_table tr").click(function () {
  ...
  if (item.eq(0).text() != "undefined" && item.eq(0).text() != "") {
    ...
    $("#DIVISIONCD").val(item.eq(0).text());
    $("#DIVISIONNAME").val(item.eq(1).text());
...
async function commit(method) {
  ...
  await axios
    .post("../DIVISIONMASTER/function/index_x.php", data)
    .then((response) => {
...
async function clearForm(form) {
  ...
  emptyRow();
  // refresh
  window.location.href = "../DIVISIONMASTER/";
  return false;
}
