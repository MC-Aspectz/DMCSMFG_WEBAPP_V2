...
// http://acc01.dmcs.biz/
if(!empty($_GET)) {
    $javaFunc = new QuoteEntry;
    if(!empty($_GET['ESTNO'])) {
        unsetSessionData();
        // libxml_use_internal_errors(TRUE);
        $query = $javaFunc->getQuote($_GET['ESTNO']);
        $data = $query;
        // $req = $javaFunc->makeRequest($query['ESTNO'], $query['ESTENTRYDT'], $query['T_AMOUNT']);
...
function printPDF($printStatic, $printDynamic, $ESTNO) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_SALEQUOTEENTRY_THA.xlsx';
        // print_r($printStatic);
        // print_r($printDynamic);
...
//                         'ESTENTRYDT' => str_replace('-', '', $data['ESTENTRYDT']),
//                         'STAFFCD' => $data['STAFFCD'],
//                         'ESTDLVCON1' => $data['ESTDLVCON1'],
//                         'ESTDLVCON2' => $data['ESTDLVCON2'],
//                         'STAFFNAME' => $data['STAFFNAME'],
//                         'ESTREM1' => $data['ESTREM1'],
//                         'ESTREM2' => $data['ESTREM2'],
//                         'ESTREM3' => $data['ESTREM3'],
//                         'CUSCURDISP' => $data['CUSCURDISP']);
//         $print = $printfunc->printStatic($param);
//         $prints = $printfunc->printDynamic($data['ESTNO']);
//         $data = $print;
//         if(!empty($prints)) {
