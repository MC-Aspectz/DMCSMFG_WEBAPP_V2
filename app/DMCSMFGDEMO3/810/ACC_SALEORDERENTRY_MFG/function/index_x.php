...
if(!empty($_GET)) {
    if(!empty($_GET['ESTNO'])) {
        ...
        if(!empty($query)) {
            ...
            $itemlist = $javaFunc->getSOLn($data['SALEORDERNO']);
            // echo '<pre>';
            // print_r($itemlist);
            // echo '</pre>';
            if(!empty($itemlist)) {
                ...
                $data['isPrint'] = 'on';
                // print_r($itemlist);
                // $param = array( 'DVW' => $itemlist,
                //                 'VATRATE' => $data['VATRATE'],
                //                 'DISCRATE' => $data['DISCRATE'],
                //                 'CUSTOMERCD' => $data['CUSTOMERCD'],
                //                 'CUSCURCD' => $data['CUSCURCD']); 
                // print_r($param);
                // $calculate = $javaFunc->calculate($param);
                // print_r($calculate);
                // $getSum = $javaFunc->getSum($param);
                // print_r($getSum);
            }
        } else { setSessionData('isPrint', 'off'); }
    }
...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'DELIVERYCD') { getDel(); }
...
function getItem() {
    ...
    if(!empty($query)) {
        ...
        $data['ITEM'][$index] = array(  'ROWNO' => $index,
                                        ...
                                        'ITEMUNITTYP2' => array_key_exists($query['ITEMUNITTYP'], $UNIT) ? $UNIT[$query['ITEMUNITTYP']]: '' ,
    ...
    echo json_encode($data['ITEM'][$index]);
}
...
function commit() {
    ...
    for ($i = 0 ; $i < count($_POST['ITEMCD']); $i++) { 
        if($_POST['ITEMCD'][$i] != '' && isset($_POST['ITEMCD'][$i])) {
            $RowParam[] = array('ROWNO' => $i+1,
                                ...
                                'SALELNDISCOUNT2' => isset($_POST['SALELNDISCOUNT2'][$i]) ? $_POST['SALELNDISCOUNT2'][$i]: '',
                                'SALELNTAXAMT' => isset($_POST['SALELNTAXAMT'][$i]) ? implode(explode(',', $_POST['SALELNTAXAMT'][$i])): '0.00',
                                'SALELNDUEDT' => !empty($_POST['SALELNDUEDT'][$i]) ? str_replace('-', '', $_POST['SALELNDUEDT'][$i]): '',
                                'SALELNPLANSHIPDT' => !empty($_POST['SALELNPLANSHIPDT'][$i]) ? str_replace('-', '', $_POST['SALELNPLANSHIPDT'][$i]): '',
                                'SALELNREM' => isset($_POST['SALELNREM'][$i]) ? $_POST['SALELNREM'][$i]: '');         
        }
    }
    // print_r($RowParam);
    $param = array( 'ESTNO' => $_POST['ESTNO'],
                    ...
                    'SALELNDUEDT' => !empty($_POST['SALELNDUEDT'][0]) ? str_replace('-', '', $_POST['SALELNDUEDT'][0]): '',
...
function setSessionArray($arr){
    $keepField = array( 'ESTNO', 'DIVISIONCD', 'DIVISIONNAME', 'SALEISSUEDT', 'CUSTOMERCD', 'BRANCHKBN', 'TAXID', 'CUSCURCD', 'CUSTOMERNAME', 'SALEORDERNO', 
                        ...
                        'SYSPVL', 'TXTLANG', 'DRPLANG', 'SALEDIVCON1', 'SALEDIVCON2', 'SALEDIVCON3', 'DEPT', 'DES', 'CDEPT', 'ANUM', 'ADDR', 'CTAXID', 'REF', 'SHV', 'DELIVERYCD', 'DELIVERYNAME',
                        'ESTCUSSTAFF', 'SALEDLVADDR1', 'DELE_PLACE_STAFF', 'SALEDLVADDR2', 'SALEPOSTCODE', 'SALEDLVTEL', 'TRANSPORTER', 'SALEDLVSTAFF', 'UNIT'
                    );
