...
// 127.0.0.1:8080
if(!empty($_GET)) {
    if(!empty($_GET['SALETRANNO'])) { // SD240700008 // SO231200001
        unsetSessionData();
    ...
    } else if(!empty($_GET['SALEORDERNO'])) { // ID230300026
        unsetSessionData();
...
function invoice() {
    ...
    try {
        ...
        if(is_array($printDynamic) && is_array($printStatic)) {
            ...
            $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_SALEENTRY_THA3_INVOICE.xlsx';
...
function taxInvoice() {
    ...
    try {
        ...
        if(is_array($printDynamic) && is_array($printStatic)) {
            ...
            $response = array();
            // Load an existing spreadsheet
            $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_SALEENTRY_THA3_TAXINVOICE.xlsx';
...
function saleVoucher() {
    ...
    try {
        ...
        // exit();
        if(!empty($printVoucher) && !empty($printStatic)) {
            ...
            $response = array();
            // Load an existing spreadsheet
            $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_SALEENTRY_THA3_SALE_VOUCHER.xlsx';
            ...
            foreach ($printVoucher as $key => $value) {
                $sheetExcel->getActiveSheet()->setCellValue('A'.$key, $value['LISTFLG'])
                                        ->setCellValue('B'.$key, $value['PROWS'])
                                        ->setCellValue('C'.$key, $value['BRKEY1'])
                                        ->setCellValue('D'.$key, $value['BRKEY2'])
                                        ->setCellValue('E'.$key, $value['ACCTRANSEQNO'])
                                        ->setCellValue('F'.$key, $value['ACCTRANSEQLN'])
                                        ->setCellValue('G'.$key, $value['ACCTRANPROCESSDT'])
                                        ->setCellValue('H'.$key, $value['ACCTRANACCCD1'])
                                        ->setCellValue('I'.$key, $value['ACCNAME1'])
                                        ->setCellValue('J'.$key, $value['ACCTRANAMT1'])
                                        ->setCellValue('K'.$key, $value['ACCTRANCURCD1'])
                                        ->setCellValue('L'.$key, $value['ACCTRANTAXTYP1'])
                                        ->setCellValue('M'.$key, $value['ACCTRANRATE1'])
                                        ->setCellValue('N'.$key, $value['ACCTRANTAXAMT1'])
                                        ->setCellValue('O'.$key, $value['ACCTRANNETAMT1'])
                                        ->setCellValue('P'.$key, $value['ACCTRANACCCD2'])
                                        ->setCellValue('Q'.$key, $value['ACCNAME2'])
                                        ->setCellValue('R'.$key, $value['ACCTRANAMT2'])
                                        ->setCellValue('S'.$key, $value['ACCTRANCURCD2'])
                                        ->setCellValue('T'.$key, $value['ACCTRANTAXCD2'])
                                        ->setCellValue('U'.$key, $value['ACCTRANRATE2'])
                                        ->setCellValue('V'.$key, $value['ACCTRANTAXAMT2'])
                                        ->setCellValue('W'.$key, $value['ACCTRANNETAMT2'])
                                        ->setCellValue('X'.$key, $value['ACCTRANPROJECT'])
                                        ->setCellValue('Y'.$key, $value['ACCTRANSTAFFCD'])
                                        ->setCellValue('Z'.$key, $value['ACCTRANSUPCD'])
                                        ->setCellValue('AA'.$key, $value['ACCTRANCUSCD'])
                                        ->setCellValue('AB'.$key, $value['ACCTRANREM'])
                                        ->setCellValue('AC'.$key, $value['ACCTRANSECTION'])
                                        ->setCellValue('AD'.$key, $value['ACCTRANDIVCD'])
                                        ->setCellValue('AE'.$key, $value['ACCTRANIFTYP'])
                                        ->setCellValue('AF'.$key, $value['ACCTRANIFNO'])
                                        ->setCellValue('AG'.$key, $value['ACCTRANTAXINVOICE'])
                                        ->setCellValue('AH'.$key, $value['ACCTRANCANCELFLG'])
                                        ->setCellValue('AI'.$key, $value['ACCTRANPRINTNO'])
                                        ->setCellValue('AJ'.$key, $value['ACCTRANPRINTLN'])
                                        ->setCellValue('AK'.$key, $value['TOTAL_DR'])
                                        ->setCellValue('AL'.$key, $value['TOTAL_CR'])
                                        ->setCellValue('AM'.$key, $value['CUSTCD'])
                                        ->setCellValue('AN'.$key, $value['CUSTNAME'])
                                        ->setCellValue('AO'.$key, $value['SALINVDATE'])
                                        ->setCellValue('AP'.$key, $value['DESCRIPTION'])
                                        ->setCellValue('AQ'.$key, $printStatic['COMPNEN']);
            }
            // --------------------------------------------------
            // Set Active Sheet to [REPORT]
            $sheetExcel->setActiveSheetIndex($sheetRpt);
