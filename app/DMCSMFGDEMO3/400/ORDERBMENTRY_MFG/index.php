...
<div class="flex flex-col h-screen">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex flex-1 overflow-y-auto paragraph">
            ...
            <form class="w-full" method="POST" id="orderbmentryMFG" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
                        ...
                        if(!empty($data['ITEM'])) { $minrow = count($data['ITEM']);
                            foreach($data['ITEM'] as $key => $value) { ?>
                                <tr class="divide-y divide-gray-200 csv" id="rowId<?=$key?>">
                                    <td class="h-6 text-sm border border-slate-700 text-center row-id" id="ROWNO_TD<?=$key?>"><?=$key?></td>
                                    <td class="h-6 text-sm border border-slate-700 whitespace-nowrap text-center" id="ALLOCBASETYP_TD<?=$key?>"><?=isset($value['ALLOCBASETYP']) ? $value['ALLOCBASETYP']: '' ?></td>
                                    <td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ITEMCD_TD<?=$key?>"><?=isset($value['ITEMCD']) ? $value['ITEMCD']: '' ?></td>
                                    <td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left whitespace-nowrap" id="ITEMNAME_TD<?=$key?>"><?=isset($value['ITEMNAME']) ? $value['ITEMNAME']: '' ?></td>
                                    <td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left whitespace-nowrap" id="ITEMSPEC_TD<?=$key?>"><?=isset($value['ITEMSPEC']) ? $value['ITEMSPEC']: '' ?></td>
                                    <td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCQTY_TD<?=$key?>"><?=isset($value['ALLOCQTY']) ? $value['ALLOCQTY']: '' ?></td>
                                    <td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ITEMUNITTYPSTR_TD<?=$key?>"><?=array_key_exists($value['ITEMUNITTYP'], $UNIT) ? $UNIT[$value['ITEMUNITTYP']]: '' ?></td>
                                    <td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCSPAREQTY_TD<?=$key?>"><?=isset($value['ALLOCSPAREQTY']) ? $value['ALLOCSPAREQTY']: '' ?></td>
                                    <td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="AQTY_TD<?=$key?>"><?=isset($value['AQTY']) ? $value['AQTY']: '' ?></td>
                                    <td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCWDQTY_TD<?=$key?>"><?=isset($value['ALLOCWDQTY']) ? $value['ALLOCWDQTY']: '' ?></td>
                                    <td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ALLOCPURORDERNOLN_TD<?=$key?>"><?=isset($value['ALLOCPURORDERNOLN']) ? $value['ALLOCPURORDERNOLN']: '' ?></td>
                                    ...
                                    <input class="hidden" id="ITEMUNITTYPSTR<?=$key?>" name="ITEMUNITTYPSTRZ[]" value="<?=array_key_exists($value['ITEMUNITTYP'], $UNIT) ? $UNIT[$value['ITEMUNITTYP']]: '' ?>">
                                ...
                                <div class="flex mb-1">
                                    ...
                                    <div class="flex w-6/12 px-1">
                                        ...
                                        <input type="text" class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                id="ALLOCPURORDERNOLN" name="ALLOCPURORDERNOLN" value="<?=!empty($data['ALLOCPURORDERNOLN']) ? $data['ALLOCPURORDERNOLN']: '' ?>"/>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        OK.click(function() {
            ...
            cols += '<td class="h-6 text-sm border border-slate-700 text-center" id="ALLOCBASETYP_TD'+index+'">'+ $('#ALLOCBASETYP').val() +'</td>';
            cols += '<td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ITEMCD_TD'+index+'">'+ $('#ITEMCD').val() +'</td>';
            cols += '<td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left whitespace-nowrap" id="ITEMNAME_TD'+index+'">'+ $('#ITEMNAME').val() +'</td>';
            cols += '<td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left whitespace-nowrap" id="ITEMSPEC_TD'+index+'">'+ $('#ITEMSPEC').val() +'</td>';
            cols += '<td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCQTY_TD'+index+'">'+ $('#ALLOCQTY').val() +'</td>';
            cols += '<td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ITEMUNITTYPSTR_TD'+index+'">'+ $('#ITEMUNITTYP option:selected').text() +'</td>';
            cols += '<td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCSPAREQTY_TD'+index+'">'+ $('#ALLOCSPAREQTY').val() +'</td>';
            cols += '<td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="AQTY_TD'+index+'">'+ $('#AQTY').val() +'</td>';
            cols += '<td class="h-6 pr-2 text-sm border border-slate-700 whitespace-nowrap text-right" id="ALLOCWDQTY_TD'+index+'">'+ $('#ALLOCWDQTY').val() +'</td>';
            cols += '<td class="h-6 pl-2 text-sm border border-slate-700 whitespace-nowrap text-left" id="ALLOCPURORDERNOLN_TD'+index+'">'+ $('#ALLOCPURORDERNOLN').val() +'</td>';
        ...
        $(document).on('click', '.pdb tbody tr', function(event) {
            ...
            if(item.eq(0).text() != 'undefined' && item.eq(0).text() != '') {
                ...
                let SYSEN_BOMLOADAPP = document.getElementById('SYSEN_BOMLOADAPP').value;
                document.getElementById('ORDER').classList[SYSEN_BOMLOADAPP == 'T' ? 'remove' : 'add']('read');
                document.getElementById('ALLOCORDERFLG').classList[SYSEN_BOMLOADAPP == 'T' ? 'add' : 'remove']('read');
