// search
const SEARCHPURORPRO_MFG = $('#SEARCHPURORPRO_MFG');
const SEARCHMATERIAL = $('#SEARCHMATERIAL');
const SEARCHITEM1 = $('#SEARCHITEM1');
const SEARCHITEM2 = $('#SEARCHITEM2');

SEARCHPURORPRO_MFG.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHPURORPRO_MFG/index.php?page=ORDERBMENTRY_MFG&ALLOCORDERTYP=' + $('#ALLOCORDERTYP').val(), 'authWindow', 'width=1200,height=600');});
SEARCHMATERIAL.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHMATERIAL/index.php?page=ORDERBMENTRY_MFG', 'authWindow', 'width=1200,height=600');});
SEARCHITEM1.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHITEM/index.php?page=ORDERBMENTRY_MFG&index=1', 'authWindow', 'width=1200,height=600');});
SEARCHITEM2.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHITEM/index.php?page=ORDERBMENTRY_MFG&index=2', 'authWindow', 'width=1200,height=600');});
...
// form
const form = document.getElementById('orderbmentryMFG');

const search_icon = [SEARCHPURORPRO_MFG, SEARCHMATERIAL, SEARCHITEM1, SEARCHITEM2, SEARCH, REMAKE];
...
async function getSearch(code, value) {
    $('#loading').show();
    return window.location.href = $('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/400/ORDERBMENTRY_MFG/index.php?'+code+'=' + value + '&ALLOCORDERTYP=' + ALLOCORDERTYP.val();        
}
...
async function getElement(code, value) {
    ...
    await axios.post('../ORDERBMENTRY_MFG/function/index_x.php', data)
    .then(response => {
...
ORDER.click(async function() {
    ...
    // let ALLOCPLANWDDT = document.getElementById('ALLOCPLANWDDT').value;
    var param = {   PFAPPCD: 'ORDERBMENTRY_MFG',
                    ROWNO: document.getElementById('ROWNO').value,
                    ...
                    ALLOCORDERFLG: document.getElementById('ALLOCORDERFLG').value,
                    ALLOCPURORDERNOLN: document.getElementById('ALLOCPURORDERNOLN').value,
                }
    // console.log(param);
    if(SYSLD_BOMLOADAPP.val() == 'PRODUCTIONORDERENTRY_MFG' && ITEMCD.val() != '' && ALLOCORDERFLG.val() == 1) {  // Production
        return $.redirect($('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/400/PRODUCTIONORDERENTRY_MFG/index.php', param);
    } else if(SYSLD_BOMLOADAPP.val() == 'ACC_PURCHSEORDERENTRY_ALLOC' && ITEMCD.val() != '' && ALLOCORDERFLG.val() == 2) { // Purchase
        return $.redirect($('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/820/ACC_PURCHSEORDERENTRY_ALLOC/index.php', param);
    } else { 
        $('#loading').hide();
    }
    // $('#loading').hide();
...
function entry() {
    ...
    document.getElementById('ALLOCORDERFLG').classList.remove('read');
...
async function clearForm(form) {
    ...
    // refresh
    window.location.href = '../ORDERBMENTRY_MFG/';
