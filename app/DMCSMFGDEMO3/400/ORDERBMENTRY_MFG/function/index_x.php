...
function search() {
    ...
    $searchfunc = new OrderBMEntryMFG;
...
function proOrPur() {
    $javaFunc = new OrderBMEntryMFG; unsetSessionkey('SYSEN_BOMLOADAPP');
...
function setALLOCPURORDERNOLN() {
    ...
    $orderSearch = $javaFunc->orderSearch($data['ORDERNO'], $data['ORDERLN'], $data['ALLOCORDERTYP'], $data['ODRQTY']);
    if(!empty($orderSearch)) {
        $data['ITEM'] = $orderSearch; 
    }
    setSessionArray($data);
    if(checkSessionData()) { $data = getSessionData(); }
}
