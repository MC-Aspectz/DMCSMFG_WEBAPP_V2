...
SEARCHCURRENCY.click(function (e) {
  e.preventDefault();
  winopened = window.open(
    $("#sessionUrl").val() +
      "/guide/" +
      $("#comcd").val() +
      "/SEARCHCURRENCY/index.php?page=EXCHANGERATEMASTER&index=1",
    "authWindow",
...
async function getCurFm(EXRATEFR) {
  ...
  await axios
    .post("../EXCHANGERATEMASTER/function/index_x.php", data)
    .then((response) => {
...
function HandlePopupResultIndex(code, result, index) {
  ...
  return (window.location.href =
    ...
    "/app/" +
    $("#comcd").val() +
    "/925/EXCHANGERATEMASTER/index.php?" +
    code +
    "=" +
    result +
    "&index=" +
