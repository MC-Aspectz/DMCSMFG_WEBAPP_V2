...
// form
const form = document.getElementById("city_entry");
...
async function commit(method) {
  ...
  await axios
    .post("../CITYENTRY/function/index_x.php", data)
    .then((response) => {
      // console.log(response.data)
...
async function clearForm(form) {
  ...
  // refresh
  window.location.href = "../CITYENTRY/";
...
function unRequired() {
  ...
  let CITYCD = document.getElementById("CITYCD");
  let CITYNAME = document.getElementById("CITYNAME");

  COUNTRYCD.classList[COUNTRYCD.value !== "" ? "remove" : "add"]("req");
  STATECD.classList[STATECD.value !== "" ? "remove" : "add"]("req");
  CITYCD.classList[CITYCD.value !== "" ? "remove" : "add"]("req");
  CITYNAME.classList[CITYNAME.value !== "" ? "remove" : "add"]("req");
}
