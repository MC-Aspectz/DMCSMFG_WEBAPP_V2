...
SEARCHSUPPLIER.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHSUPPLIER/index.php?page=ACC_WHTMETHOD2', 'authWindow', 'width=1200,height=600');});
SEARCHACCOUNT1.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHACCOUNT/index.php?page=ACC_WHTMETHOD2&index=1', 'authWindow', 'width=1200,height=600');});
SEARCHACCOUNT2.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHACCOUNT/index.php?page=ACC_WHTMETHOD2&index=2', 'authWindow', 'width=1200,height=600');});
SEARCHACCOUNT3.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHACCOUNT/index.php?page=ACC_WHTMETHOD2&index=3', 'authWindow', 'width=1200,height=600');});

const serach_icon = [ SEARCHSUPPLIER, SEARCHACCOUNT1, SEARCHACCOUNT2, SEARCHACCOUNT3];
...
const input_serach = [SUPPLIERCD, ACC_CD1, ACC_CD2, ACC_CD3];
...
// form
const form = document.getElementById('supplierwithholdtax');
...
SUPPLIERCD.on('keyup change', function(e) {
    if(e.type === 'change') {
        getSearch('SUPPLIERCD', SUPPLIERCD.val());
...
async function getSearch(code, value) {
    $('#loading').show();
    return window.location.href = $('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/905/ACC_WHTMETHOD2/index.php?'+code+'=' + value;  
}
...
async function commit(method) {
    ...
    await axios.post('../ACC_WHTMETHOD2/function/index_x.php', data)
    .then(response => {
...
function unRequired() {
    ...
    SUPPLIERCD.classList[SUPPLIERCD.value !== '' ? 'remove' : 'add']('req');
...
async function clearForm(form) {
    ...
    // emptyRow();
    // refresh
    window.location.href = '../ACC_WHTMETHOD2/';
    return false;
}

function emptyRow() {
    for (var i = 0; i < 8; i++) {
        $('table tbody').append('<tr class="divide-y divide-gray-200">' +
